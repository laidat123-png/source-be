...
const AdminCheckHandler = require('../handlers/adminCheckHandler');
const ProductStockHandler = require('../handlers/productStockHandler');
...
exports.createOrders = async (req, res, next) => {
    try {
        const adminCheckHandler = new AdminCheckHandler();
        const productStockHandler = new ProductStockHandler();
        const createOrderHandler = new CreateOrderHandler();

        adminCheckHandler.setNext(productStockHandler).setNext(createOrderHandler);

        const newOrder = await adminCheckHandler.handle(req);
        res.status(200).json({
            status: "success",
            order: newOrder
        });
